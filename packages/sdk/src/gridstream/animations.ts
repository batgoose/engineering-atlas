...
export const GRIDSTREAM_KEYFRAMES = `
  ...
  @keyframes markerStandUp{0%{transform:translateY(12px) scaleX(1.28) scaleY(.08);opacity:.55}62%{transform:translateY(4px) scaleX(1.01) scaleY(.9);opacity:.95}100%{transform:translateY(0) scaleX(.94) scaleY(1.32);opacity:1}}
  @keyframes cardStandUp{0%{transform:translateY(16px) scaleX(1.08) scaleY(.2);opacity:.6}64%{transform:translateY(6px) scaleX(1.01) scaleY(.92);opacity:.95}100%{transform:translateY(0) scaleX(.97) scaleY(1.28);opacity:1}}
  ...
  @keyframes playSweepAnim{0%{transform:translateX(0);opacity:0}5%{opacity:1}80%{opacity:.5}100%{transform:translateX(1600%);opacity:0}}
`;
...
export const GRIDSTREAM_BASE_STYLES = `
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .tab-btn::before{content:'■';font-size:5px;margin-right:6px;color:${C.textMuted};vertical-align:middle;transition:color .15s,text-shadow .15s}
  .tab-btn:hover{color:${C.cyanDim}}
  .tab-btn:hover::before{color:${C.cyanDim}}
  .tab-btn.active{color:${C.cyan};background:${C.panel};border-color:${C.panelBorder}}
  .tab-btn.active::before{color:${C.cyan};text-shadow:0 0 8px rgba(0,229,255,.9);animation:possGlow 2s ease-in-out infinite}
  .tab-btn.active::after{content:'';position:absolute;bottom:-1px;left:0;right:0;height:2px;background:${C.panel}}
  .play-sweep{position:absolute;top:0;bottom:0;left:0;width:80px;pointer-events:none;z-index:5;background:linear-gradient(90deg,transparent,rgba(0,229,255,.25),transparent);animation:playSweepAnim .65s ease-out forwards}
