...
const EMPTY_SPLIT_AGG = {
  games: 0, passYds: 0, passTds: 0, passFirstDowns: 0,
  rushYds: 0, rushTds: 0, rushFirstDowns: 0,
  recYds: 0, recTds: 0, recFirstDowns: 0,
  fumbles: 0, fumblesLost: 0, forcedFumbles: 0, ppr: 0,
  defTackles: 0, defSacks: 0, defQbHits: 0, defPd: 0,
  defInts: 0, defIntTds: 0, defTds: 0,
};
const EMPTY_SPLITS = {
...
const SCHOOL_CONF: Record<string, string> = {
  // SEC
  alabama: 'SEC', lsu: 'SEC', georgia: 'SEC', florida: 'SEC', auburn: 'SEC',
  tennessee: 'SEC', 'mississippi state': 'SEC', 'ole miss': 'SEC',
  'south carolina': 'SEC', kentucky: 'SEC', vanderbilt: 'SEC', arkansas: 'SEC',
  missouri: 'SEC', texas: 'SEC', oklahoma: 'SEC', 'texas a&m': 'SEC',
  // Big Ten
  'ohio state': 'Big Ten', michigan: 'Big Ten', 'penn state': 'Big Ten',
  'michigan state': 'Big Ten', wisconsin: 'Big Ten', iowa: 'Big Ten',
  minnesota: 'Big Ten', nebraska: 'Big Ten', illinois: 'Big Ten',
  indiana: 'Big Ten', purdue: 'Big Ten', rutgers: 'Big Ten',
  maryland: 'Big Ten', northwestern: 'Big Ten', ucla: 'Big Ten',
  usc: 'Big Ten', washington: 'Big Ten', oregon: 'Big Ten',
  // ACC
  clemson: 'ACC', 'florida state': 'ACC', miami: 'ACC',
  'north carolina': 'ACC', 'nc state': 'ACC', 'virginia tech': 'ACC',
  virginia: 'ACC', 'wake forest': 'ACC', duke: 'ACC',
  'boston college': 'ACC', syracuse: 'ACC', pittsburgh: 'ACC',
  'georgia tech': 'ACC', louisville: 'ACC', cal: 'ACC', stanford: 'ACC', smu: 'ACC',
  // Big 12 (current + recent)
  'oklahoma state': 'Big 12', 'kansas state': 'Big 12', kansas: 'Big 12',
  baylor: 'Big 12', tcu: 'Big 12', 'texas tech': 'Big 12',
  'iowa state': 'Big 12', 'west virginia': 'Big 12', cincinnati: 'Big 12',
  ucf: 'Big 12', houston: 'Big 12', byu: 'Big 12',
  'arizona state': 'Big 12', arizona: 'Big 12', utah: 'Big 12', colorado: 'Big 12',
  // Pac-12 (pre-2024 era)
  'washington state': 'Pac-12', 'oregon state': 'Pac-12',
  // MWC
  'boise state': 'MWC', 'fresno state': 'MWC', nevada: 'MWC',
  'utah state': 'MWC', wyoming: 'MWC', 'san diego state': 'MWC',
  'air force': 'MWC', unlv: 'MWC', 'colorado state': 'MWC',
  // AAC
  memphis: 'AAC', tulsa: 'AAC', tulane: 'AAC',
  // Independents / others
  'notre dame': 'Ind.', army: 'Ind.', navy: 'Ind.',
  liberty: 'CUSA', 'james madison': 'Sun Belt',
};
...
function buildCollegeEntries(
  ...
  const schools = profile.college.split(';').map((s) => s.trim()).filter(Boolean);
...
export default async function GridstreamPlayerDetailPage({
  ...
  if (numericPlayerId) {
    try {
      profile = await fetchGridstreamPlayerProfile({ apiBase: API_BASE, playerId: numericPlayerId });
    } catch (error) {
      dataWarning = error instanceof Error ? error.message : 'Failed to load live profile.';
    }

    if (profile) {
      const latestSeason = profile.seasonsPlayed.length
        ? Math.max(...profile.seasonsPlayed)
        : null;
  ...
  return (
    <main className="gs-players-page">
      <div className="gs-players-shell">
        <header className="gs-players-header gs-player-detail-header">
          {profile.headshotUrl && (
            <div
              ...
              style={profile.currentTeamColors?.primary ? {
        ...
        <div className="gs-player-detail-panels-grid">
            ...
            <div className="gs-player-detail-meta">
                ...
                <div className="gs-player-detail-item-value">
                  ...
                  {profile.birthDate && (
                    <span className="gs-player-detail-item-sub">
                      {new Date(`${profile.birthDate}T00:00:00Z`).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' })}
                    </span>
                  )}
                </div>
              </article>
              ...
              <article className="gs-player-detail-item">
                ...
                <div className="gs-player-detail-item-value">
                  ...
                  {profile.draftOverall != null && (
                    <span className="gs-player-detail-item-sub">#{profile.draftOverall} overall</span>
                  )}
                </div>
              </article>
              <article className="gs-player-detail-item">
              ...
              {profile.maddenRating && (() => {
                ...
                const attrs: Attr[] = [
                  ...
                  ...(isQB ? [{ label: 'Throw Pwr', value: m.throwPower }, { label: 'Accel', value: m.acceleration }] : []),
                  ...(isRB || isWRTE ? [{ label: 'Catching', value: m.catching }, { label: 'Accel', value: m.acceleration }] : []),
                  ...(isWRTE ? [{ label: 'Route Run', value: m.routeRunning }] : []),
                  ...(isOL ? [{ label: 'Run Block', value: m.runBlock }, { label: 'Pass Block', value: m.passBlock }] : []),
                  ...(isDL ? [{ label: 'Tackle', value: m.tackle }, { label: 'Pwr Moves', value: m.powerMoves }, { label: 'Fin Moves', value: m.finesseMoves }] : []),
                  ...(isLB ? [{ label: 'Tackle', value: m.tackle }, { label: 'Hit Power', value: m.hitPower }, { label: 'Accel', value: m.acceleration }] : []),
                  ...(isDB ? [{ label: 'Man Cover', value: m.manCoverage }, { label: 'Zone Cover', value: m.zoneCoverage }, { label: 'Hit Power', value: m.hitPower }] : []),
                  ...(!isQB && !isRB && !isWRTE && !isOL && !isDL && !isLB && !isDB && !isK ? [{ label: 'Accel', value: m.acceleration }] : []),
                ].filter((a) => a.value != null);
                return (
                  <article className="gs-player-detail-item gs-player-detail-item--madden">
                    ...
                    <div className="gs-player-detail-item-value">
                      <span className={`gs-player-madden-ovr gs-player-madden-ovr--${maddenOvrTier(m.overall)}`}>
                        {m.overall}
                      </span>
                      <span className="gs-player-detail-item-sub">
                        Madden {m.maddenYear}
                    ...
                    {attrs.length > 0 && (
                      <div className="gs-player-madden-popup">
                        <div className="gs-player-madden-popup-title">Madden {m.maddenYear}</div>
          ...
          {(() => {
            ...
            const hasMeasurements = [
              combine.fortyYard, combine.benchPress, combine.verticalJump,
              combine.broadJump, combine.threeCone, combine.shuttle,
              combine.armLength, combine.handSize,
            ].some((v) => v != null);
                ...
                <div className="gs-player-detail-combine-grid">
                  {combine.fortyYard != null && (
                    <article className="gs-player-detail-combine-item">
                      ...
                      <div className="gs-player-detail-combine-value">{combine.fortyYard.toFixed(2)}s</div>
                    </article>
                  )}
                  {combine.benchPress != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">Bench Press</div>
                      <div className="gs-player-detail-combine-value">{combine.benchPress} reps</div>
                    </article>
                  )}
                  {combine.verticalJump != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">Vertical</div>
                      <div className="gs-player-detail-combine-value">{combine.verticalJump}&quot;</div>
                    </article>
                  )}
                  {combine.broadJump != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">Broad Jump</div>
                      <div className="gs-player-detail-combine-value">{combine.broadJump}&quot;</div>
                    </article>
                  )}
                  {combine.threeCone != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">3-Cone</div>
                      <div className="gs-player-detail-combine-value">{combine.threeCone.toFixed(2)}s</div>
                    </article>
                  )}
                  {combine.shuttle != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">Shuttle</div>
                      <div className="gs-player-detail-combine-value">{combine.shuttle.toFixed(2)}s</div>
                    </article>
                  )}
                  {combine.armLength != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">Arm Length</div>
                      <div className="gs-player-detail-combine-value">{combine.armLength}&quot;</div>
                    </article>
                  )}
                  {combine.handSize != null && (
                    <article className="gs-player-detail-combine-item">
                      <div className="gs-player-detail-combine-label">Hand Size</div>
                      <div className="gs-player-detail-combine-value">{combine.handSize}&quot;</div>
                    </article>
                  )}
                </div>
              </details>
                        ...
                        return (
                          <div key={entry.id} className="gs-player-detail-college-row">
                            ...
                            <span className="gs-player-detail-college-name">
                              ...
                              {conf && <span className="gs-player-detail-college-conf">({conf})</span>}
                            </span>
                            {entry.isRedshirt && (
            ...
            <details className="hud-panel gs-player-detail-awards">
              ...
              <div className="gs-player-detail-awards-list">
                {[...profile.awards]
                  ...
                  .map((award) => (
                    <div key={`${award.season}-${award.espnAwardId}`} className="gs-player-detail-award-row">
          ...
          </div>{/* end gs-player-detail-panels-left */}

          {/* Right column: Contract History */}
          {(() => {
            const contracts = [...(profile.contracts ?? [])]
              .sort((a, b) => (b.yearSigned ?? 0) - (a.yearSigned ?? 0));
        ...
        </div>{/* end gs-player-detail-panels-grid */}
