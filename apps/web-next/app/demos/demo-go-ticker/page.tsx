        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            ...
            <p className={`${typography.bodySmall} mt-2`}>
              Connected to the Go WebSocket service via a framework-agnostic SDK. Events stream in
              as typed envelopes — game updates, plays, and scoring events rendered in real time.
            </p>
          </div>
          ...
          <div className="lg:col-span-2 bg-atlas-darker/60 p-4 rounded-sm border border-slate-800/50">
            {recentPlays.length === 0 ? (
              <div className="p-8 text-center font-mono text-xs text-slate-600 uppercase tracking-wider">
                {isConnected ? 'Listening for game events...' : 'Awaiting connection...'}
              </div>
            ) : (
...
function TickerRow({ play }: { play: MissionLogEntry }) {
  ...
  return (
    ...
    >
      <div>
        {play.team && <span className="font-display font-bold text-white mr-2">{play.team}</span>}
      ...
      <div className="flex items-center gap-3">
        ...
        {play.epa !== 0 && (
          <span
            className={`font-mono text-xs ${play.epa > 0 ? 'text-emerald-400' : 'text-red-400'}`}
          >
            {play.epa > 0 ? '+' : ''}
            {play.epa.toFixed(1)}
          </span>
