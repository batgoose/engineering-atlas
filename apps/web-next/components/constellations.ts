import * as THREE from 'three';
import { SVGLoader } from 'three-stdlib';

// ============================================================
// Types
// ============================================================

export interface ConstellationPoint {
  x: number;
  y: number;
}

export interface ConstellationDefinition {
  center: THREE.Vector3;
  scale: number;
  svgString: string;
  color: string;
  starPoints: ConstellationPoint[];
  radius: number;
}

// ============================================================
// SVG ASSETS
// ============================================================

const SVGS = {
  CHART: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21H7.8c-1.68 0-2.52 0-3.162-.327a3 3 0 0 1-1.311-1.311C3 18.72 3 17.88 3 16.2V3m3 12 4-4 4 4 6-6m0 0v4m0-4h-4"/></svg>`,
  CLOUD: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -64.71 306.67 306.67"><path d="M41.28 83.23c1.26-3.3 2.46-6.2 3.5-9.16 4-11.39 13-16.23 24-18.13 3.34-.58 6.85-.22 10.29-.26h6.38c.25-4.44.71-8.36.63-12.28-.12-5.51 2.1-9.82 5.47-14C106.7 10.54 126.84 1.49 150.4.08c11.91-.71 23.08 3.37 33.37 9.52 9.66 5.78 16.42 14.08 21.2 24.14.72 1.52 1.6 3 2.64 4.88 10.77-2.21 21.08-2 30 5.6a44.11 44.11 0 0 1 15.49 27.37h6.18c5.7.06 11.38-.22 17 1.34 8.39 2.3 15.06 6.66 19.71 14.09 6.67 10.63 11 21.94 10.65 34.73-.31 11.72-5 20.84-15 27.4a67.82 67.82 0 0 1-25.27 10.22 218.2 218.2 0 0 1-23.67 2.75 38 38 0 0 1-10.19-1.18c-4.74-1-9 0-13.36 1.87-7.58 3.28-15.13 6.66-22.88 9.49a70.68 70.68 0 0 1-14.88 3.7 120.42 120.42 0 0 1-33.33.09c-10.09-1.48-18.93-6.24-27.31-11.86-1.65-1.11-3.32-2.19-5.28-3.5-2.93 1.68-5.78 3.5-8.79 5-2.82 1.4-5.77 2.52-8.71 3.64a57.71 57.71 0 0 1-18.44 3.66c-9.45.38-18.84.75-28.17-1.66-22.79-5.86-39.45-19.08-48.67-40.85-3.94-9.29-3.27-19.06.18-28.51a17.64 17.64 0 0 1 3.66-5.71c7.75-8.3 17.17-13.18 28.8-13.07Zm74.85 64.11c5.7 6.59 13 10.5 20.82 13.3 11.57 4.11 23.62 4.44 35.66 3.24 10.28-1 20.41-3 29.54-8.32 3.21-1.87 6.57-3.5 9.78-5.37a10.77 10.77 0 0 1 8.24-1.24c15.54 3.57 30.82 1.14 45.89-2.59 6.88-1.71 13.84-3.7 19.79-8 4.81-3.49 8.72-7.76 9.77-13.63 1.31-7.34 1.88-14.8-2.24-21.72a39.64 39.64 0 0 1-2.82-6.26C288 90 282.66 86.4 276.33 84a38.16 38.16 0 0 0-8.33-2c-7.15-1-14.29-1.24-21.14 1.71-2.16.93-3.92.23-5.35-1.45s-1.78-3.6 0-5.33 1.52-3.6 1.32-5.6c-1-9.85-6-17.08-14.47-22-2.54-1.48-5.05-2.48-8.1-2.08-2.81.37-5.8-.14-8.49.56-2.93.77-5.62 2.49-8.24 3.72-3.39-1.06-4.78-3.19-5.2-6.38-.35-2.78-.65-5.82-2-8.21-5.85-10.59-14.1-18.42-25.84-22.6a53 53 0 0 0-31.18-1.54 69.85 69.85 0 0 0-37.41 22.79 23.63 23.63 0 0 0-5.79 12.27c-.53 4-.71 8-1.18 12-.61 5.13-3 7-8 6.62-3.7-.27-7.46-1-11.12-.63-13.51 1.25-20.39 6.81-23.27 18.92-.33 1.38-.49 2.81-.8 4.2a10 10 0 0 1-4.48 6.75c-1.77-.38-4-.81-6.14-1.32-10.94-2.54-25.68 5.4-29.34 15.87a17.19 17.19 0 0 0-.75 3.34 30.5 30.5 0 0 0 8 26.11c9.37 10.36 21.09 16.28 34.59 19.19 10 2.19 19.91 1.31 29.91.31 8.14-.81 15.72-2.95 22.48-7.67a23.84 23.84 0 0 1 10.12-4.21Z"/><path d="M116.13 147.34a23.84 23.84 0 0 0-10.13 4.21c-6.76 4.72-14.34 6.86-22.48 7.67-10 1-19.93 1.88-29.91-.31-13.5-3-25.22-8.83-34.66-19.24a30.5 30.5 0 0 1-8-26.11 17.19 17.19 0 0 1 .75-3.34c3.66-10.47 18.4-18.41 29.34-15.87 2.18.51 4.37.94 6.14 1.32a10 10 0 0 0 4.48-6.75c.31-1.39.47-2.82.8-4.2 2.88-12.11 9.76-17.67 23.27-18.92 3.66-.34 7.42.36 11.12.63 5.07.37 7.42-1.49 8-6.62.47-4 .65-8 1.18-12a23.63 23.63 0 0 1 5.79-12.27 69.85 69.85 0 0 1 37.45-22.85 53 53 0 0 1 31.18 1.54c11.74 4.18 20 12 25.84 22.6 1.31 2.39 1.61 5.43 2 8.21.42 3.19 1.81 5.32 5.2 6.38 2.62-1.23 5.31-2.95 8.24-3.72 2.69-.7 5.68-.19 8.49-.56 3.05-.4 5.56.6 8.1 2.08 8.51 4.95 13.47 12.18 14.47 22 .2 2 .33 4-1.32 5.6s-1.49 3.58 0 5.33 3.19 2.38 5.35 1.45c6.85-3 14-2.67 21.14-1.71a38.16 38.16 0 0 1 8.32 2c6.33 2.4 11.68 6 14.23 12.73a39.64 39.64 0 0 0 2.82 6.26c4.12 6.92 3.55 14.38 2.24 21.72-1.05 5.87-5 10.14-9.77 13.63-6 4.32-12.91 6.31-19.79 8-15.07 3.73-30.35 6.16-45.89 2.59a10.77 10.77 0 0 0-8.24 1.24c-3.21 1.87-6.57 3.5-9.78 5.37-9.13 5.33-19.26 7.3-29.54 8.32-12 1.2-24.09.87-35.66-3.24-7.79-2.67-15.07-6.58-20.77-13.17Z" style="fill:#fff"/></svg>`,
  DATABASE: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M459.991 82.262c-5.818-13.388-15.866-24.916-28.61-34.854-19.179-14.896-44.758-26.522-74.674-34.691C326.789 4.575 292.498 0 256 0c-55.622.036-106.071 10.548-143.848 28.374-18.889 8.959-34.692 19.76-46.31 32.713-5.791 6.48-10.52 13.542-13.832 21.176-3.304 7.634-5.146 15.866-5.138 24.29V405.44c-.008 8.423 1.834 16.665 5.138 24.29 5.818 13.388 15.866 24.915 28.61 34.863 19.18 14.896 44.757 26.523 74.675 34.692C185.211 507.425 219.503 511.992 256 512c55.632-.046 106.07-10.546 143.848-28.383 18.888-8.958 34.691-19.751 46.309-32.712 5.792-6.472 10.52-13.534 13.833-21.176 3.304-7.624 5.146-15.867 5.137-24.29V106.552c.01-8.424-1.832-16.656-5.136-24.29zM436.282 405.44c-.009 4.293-.889 8.478-2.768 12.843-3.268 7.597-9.849 15.766-19.861 23.545-14.967 11.7-37.369 22.257-64.526 29.626-27.158 7.388-59.054 11.709-93.128 11.7-51.909.046-98.818-10.084-131.503-25.606-16.357-7.715-29.064-16.774-37.142-25.86-4.066-4.538-6.989-9.058-8.868-13.406-1.88-4.366-2.76-8.55-2.769-12.843v-49.36c16.684 15.25 40.111 27.43 68.13 36.435 32.304 10.32 70.789 16.247 112.152 16.256 55.16-.027 105.164-10.493 142.306-28.029 14.931-7.08 27.693-15.368 37.976-24.798v49.497zm0-84.242c-2.051 4.402-4.983 8.814-8.94 13.27-12.78 14.396-35.917 27.948-65.77 37.442-29.844 9.548-66.287 15.23-105.572 15.222-52.372.027-99.726-10.14-133.046-25.95-16.674-7.87-29.763-17.137-38.294-26.714-3.958-4.456-6.89-8.868-8.942-13.26v-64.754c16.684 15.249 40.111 27.43 68.13 36.434 32.304 10.311 70.789 16.238 112.152 16.248 55.16-.028 105.164-10.492 142.306-28.03 14.931-7.08 27.693-15.357 37.976-24.788v64.88zm0-99.626c-2.051 4.402-4.983 8.813-8.94 13.261-12.78 14.395-35.917 27.956-65.77 37.441-29.844 9.548-66.287 15.231-105.572 15.222-52.372.028-99.726-10.138-133.046-25.941-16.674-7.87-29.763-17.146-38.294-26.722-3.958-4.448-6.89-8.859-8.942-13.261v-64.754c16.684 15.258 40.111 27.43 68.13 36.443 32.304 10.32 70.789 16.247 112.152 16.247 55.16-.027 105.164-10.484 142.306-28.029 14.931-7.08 27.693-15.358 37.976-24.789v64.882zm0-99.636c-2.051 4.403-4.983 8.823-8.94 13.271-12.78 14.405-35.917 27.956-65.77 37.451-29.844 9.548-66.287 15.23-105.572 15.222-52.372.027-99.726-10.14-133.046-25.95-16.674-7.879-29.763-17.147-38.294-26.722-3.958-4.448-6.89-8.868-8.942-13.262v-15.394c.01-4.294.89-8.469 2.769-12.835 3.268-7.607 9.848-15.766 19.86-23.554 14.968-11.7 37.369-22.256 64.526-29.617 27.158-7.398 59.053-11.709 93.127-11.7 51.91-.046 98.819 10.085 131.504 25.606 16.356 7.706 29.054 16.765 37.142 25.86 4.066 4.538 6.988 9.05 8.868 13.406 1.878 4.366 2.758 8.541 2.768 12.835v15.383z" style="fill:#000"/></svg>`,
  GEAR: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 56 56"><path d="M28 50.828c.61 0 1.172-.07 1.805-.117l1.36 2.578c.257.516.773.797 1.405.703.586-.117 1.008-.539 1.078-1.172l.422-2.836c1.149-.304 2.297-.75 3.422-1.242l2.11 1.899c.445.422 1.007.515 1.593.187.516-.281.727-.844.61-1.453l-.586-2.813a21.53 21.53 0 0 0 2.789-2.32l2.625 1.102c.586.234 1.125.117 1.57-.422.399-.422.445-1.031.117-1.547l-1.523-2.438c.703-.984 1.266-2.039 1.805-3.163l2.86.14c.608.023 1.124-.305 1.335-.89.187-.587 0-1.149-.492-1.524l-2.227-1.781c.305-1.125.54-2.344.633-3.61l2.719-.843c.586-.211.96-.657.96-1.29 0-.609-.374-1.054-.96-1.265l-2.72-.867c-.093-1.266-.327-2.438-.632-3.586l2.226-1.805c.493-.351.68-.89.493-1.476-.211-.586-.727-.915-1.336-.891l-2.86.117c-.538-1.125-1.1-2.18-1.804-3.164l1.524-2.437c.327-.493.28-1.102-.118-1.524-.445-.539-.984-.68-1.57-.422l-2.625 1.055a25.884 25.884 0 0 0-2.79-2.297l.587-2.789c.117-.633-.094-1.195-.61-1.477-.586-.304-1.148-.257-1.593.211l-2.11 1.852a27.105 27.105 0 0 0-3.422-1.219l-.422-2.836c-.07-.61-.492-1.054-1.078-1.148-.633-.094-1.148.164-1.406.68l-1.36 2.578c-.632-.047-1.195-.094-1.804-.094-.633 0-1.195.047-1.852.094l-1.335-2.579c-.258-.515-.774-.773-1.43-.68-.586.095-.985.54-1.078 1.15l-.399 2.812c-1.172.351-2.32.75-3.422 1.242l-2.109-1.852c-.469-.468-1.031-.515-1.594-.21-.515.28-.726.843-.61 1.476l.587 2.79a24.044 24.044 0 0 0-2.813 2.296l-2.601-1.055c-.586-.258-1.149-.117-1.594.422a1.235 1.235 0 0 0-.117 1.5L9.18 15.04c-.703.984-1.266 2.04-1.805 3.164l-2.883-.117c-.61-.023-1.125.305-1.312.89-.188.587-.024 1.126.468 1.477l2.25 1.805c-.304 1.148-.562 2.32-.632 3.586l-2.72.867c-.608.187-.937.633-.937 1.266 0 .632.329 1.078.938 1.289l2.719.867c.07 1.242.328 2.46.633 3.586L3.649 35.5c-.493.375-.657.938-.47 1.523.188.586.704.914 1.313.891l2.883-.14c.54 1.124 1.102 2.18 1.781 3.163l-1.5 2.438c-.351.516-.304 1.125.094 1.547.445.539 1.008.656 1.594.422l2.601-1.102c.868.867 1.829 1.64 2.813 2.32l-.586 2.813c-.117.61.094 1.172.61 1.453.585.328 1.148.234 1.593-.187l2.086-1.899c1.125.492 2.273.938 3.445 1.242l.399 2.836c.093.633.492 1.055 1.078 1.172.656.094 1.172-.187 1.43-.703l1.335-2.578c.633.047 1.22.117 1.852.117ZM33.883 26.5c-1.102-2.953-3.258-4.57-5.953-4.57-.399 0-.867.047-1.617.21l-6.727-11.484C22.094 9.414 24.953 8.734 28 8.734c10.219 0 18.234 7.758 18.961 17.766ZM8.969 28c0-6.516 3.07-12.258 7.922-15.726l6.773 11.554c-1.266 1.313-1.851 2.742-1.851 4.266 0 1.476.562 2.836 1.851 4.195l-6.937 11.344C11.992 40.14 8.969 34.469 8.969 28Zm16.148.07c0-1.593 1.36-2.836 2.86-2.836 1.593 0 2.906 1.243 2.906 2.836 0 1.57-1.313 2.86-2.906 2.86-1.5 0-2.86-1.29-2.86-2.86ZM28 47.266c-3.117 0-6.023-.727-8.578-1.992L26.312 34c.727.188 1.22.234 1.618.234 2.718 0 4.875-1.664 5.953-4.687H46.96c-.75 9.96-8.766 17.719-18.961 17.719Z"/></svg>`,
  MONITOR: `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="800" viewBox="0 0 32 32"><path d="M28 2.25H4A2.754 2.754 0 0 0 1.25 5v16A2.754 2.754 0 0 0 4 23.75h8.806l-2.167 4.5h-4.64a.75.75 0 0 0 0 1.5h20a.75.75 0 0 0 0-1.5h-4.537l-2.25-4.5h8.787a2.754 2.754 0 0 0 2.75-2.75V5a2.754 2.754 0 0 0-2.75-2.75zm-8.213 26h-7.482l2.166-4.5h3.065zM29.25 21A1.252 1.252 0 0 1 28 22.25H4A1.252 1.252 0 0 1 2.75 21V5c.001-.69.56-1.249 1.25-1.25h24c.69.001 1.249.56 1.25 1.25zM9.026 13.252l3.538-4.044a.75.75 0 0 0-1.128-.989l-.001.001-4 4.572a.746.746 0 0 0 .035 1.024l4 4a.75.75 0 0 0 1.061-1.061zm11.538-5.031a.749.749 0 0 0-1.312.496c0 .189.07.361.184.493l-.001-.001 3.537 4.043-3.504 3.504a.75.75 0 0 0 1.063 1.061l4-4a.75.75 0 0 0 .032-1.025l.001.001z"/></svg>`,
  LIGHTBULB: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M425.813 155.307C417.28 74.24 352.427 9.387 272.213.853 267.093.853 261.973 0 256 0c-5.973 0-11.093.853-16.213.853-80.214 9.387-145.067 74.24-153.6 154.454-5.973 54.613 13.653 108.373 52.907 145.92 21.333 20.48 34.987 44.373 40.107 67.413v100.693c0 5.12 3.413 8.533 8.533 8.533h12.8l15.36 20.48C222.72 506.88 232.107 512 243.2 512h25.6c11.093 0 21.333-5.12 27.307-13.653l15.36-20.48h12.8c5.12 0 8.533-3.413 8.533-8.533V368.64c4.267-23.04 18.773-46.933 40.107-67.413 39.253-38.4 58.88-91.307 52.906-145.92zm-203.946 58.026H204.8c-9.387 0-17.067-7.68-17.067-17.067s7.68-17.067 17.067-17.067 17.067 7.68 17.067 17.067v17.067zm51.2 145.067h-34.133v-128h34.133v128zm42.666 17.067v17.067h-25.6c-5.12 0-8.533 3.413-8.533 8.533 0 5.12 3.413 8.533 8.533 8.533h25.6v17.067h-51.2c-5.12 0-8.533 3.413-8.533 8.533s3.413 8.533 8.533 8.533h51.2V460.8H196.267v-17.067h25.6c5.12 0 8.533-3.413 8.533-8.533s-3.413-8.533-8.533-8.533h-25.6V409.6h51.2c5.12 0 8.533-3.413 8.533-8.533 0-5.12-3.413-8.533-8.533-8.533h-51.2v-17.067h119.466zm-33.28 112.64c-3.413 4.267-8.533 6.827-13.653 6.827h-25.6c-5.12 0-10.24-2.56-13.653-6.827l-7.68-10.24h68.267l-7.681 10.24zm78.507-199.68c-21.333 21.333-36.693 45.227-43.52 69.973h-27.307v-128H307.2c18.773 0 34.133-15.36 34.133-34.133 0-18.773-15.36-34.133-34.133-34.133-18.773 0-34.133 15.36-34.133 34.133v17.067h-34.133v-17.067c0-18.773-15.36-34.133-34.133-34.133s-34.133 15.36-34.133 34.133c0 18.773 15.36 34.133 34.133 34.133h17.067v128H194.56c-6.827-24.747-22.187-48.64-43.52-69.973-35.84-34.133-52.907-81.92-47.787-131.413 8.533-72.533 66.56-130.56 139.093-139.093 4.267 0 8.533-.853 13.653-.853 5.12 0 9.387.853 13.653.853 72.533 8.533 131.413 66.56 139.093 139.093 5.122 49.493-11.945 97.279-47.785 131.413zm-70.827-75.094v-17.067c0-9.387 7.68-17.067 17.067-17.067s17.067 7.68 17.067 17.067-7.68 17.067-17.067 17.067h-17.067z"/></svg>`,
  TOOLS: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 65 65"><g fill="none" fill-rule="evenodd" stroke="#6B6C6E" stroke-width="2"><path d="M26.6 19.1c1.9-4.7 3.4-7.9-2.8-14.1-2.6-2.6-6.1-3.9-9.5-3.8l5.1 5.1-2.7 10-10 2.7-5.1-5.1c-.1 3.4 1.2 6.8 3.8 9.5 6.2 6.2 9.4 4.8 14.1 2.8M38.6 45.3c-1.9 4.7-3.4 7.8 2.8 14.1 2.6 2.6 6 3.9 9.5 3.8L46 58.3l2.7-10 10-2.7 4.9 4.9c.1-3.4-1.2-6.9-3.8-9.5-6.2-6.2-9.4-4.8-14-2.9M26.8 33.5l-9.3-9.4M40.7 47.3l-9.5-9.5M41.2 33.7l6.6 6.6M24.5 17.1l6.3 6.2M5.1 62.5l-2.8-2.8 5.6-8.5 3.6-2.1 17.6-17.7 4.3 4.3-17.7 17.7-2.1 3.5-8.5 5.6Z"/><path d="m41.2 33.6 21.9-21.9c.8-.8.8-2 0-2.8L56 1.8c-.8-.8-2-.8-2.8 0L31.3 23.7c-.6.6-.7 1.5-.4 2.3-.4.1-3.2 1.3-3.2 1.3-.8.8-.8 2 0 2.8l7.1 7.1c.8.8 2 .8 2.8 0 0 0 1.2-2.8 1.3-3.2.7.3 1.6.2 2.3-.4ZM53.9 6.7 36.2 24.4M58.1 10.9 40.5 28.6"/></g></svg>`,
};

// ============================================================
// CONSTELLATION DEFINITIONS
// ============================================================

export const CONSTELLATIONS: Record<string, ConstellationDefinition> = {
  Frontend: {
    center: new THREE.Vector3(-8, 3.5, 0),
    scale: 2.5,
    svgString: SVGS.MONITOR,
    radius: 4,
    color: '#00ffff',
    starPoints: [
      { x: -0.8, y: 0.7 },
      { x: 0.8, y: 0.7 },
      { x: 0.8, y: -0.4 },
      { x: -0.8, y: -0.4 },
      { x: 0, y: -0.9 },
    ],
  },

  Backend: {
    center: new THREE.Vector3(8, 3.5, 0),
    scale: 2.5,
    svgString: SVGS.DATABASE,
    radius: 4,
    color: '#cc44ff',
    starPoints: [
      { x: 0, y: 0.9 },
      { x: -0.8, y: 0.5 },
      { x: 0.8, y: 0.5 },
      { x: -0.8, y: -0.5 },
      { x: 0.8, y: -0.5 },
      { x: 0, y: -0.9 },
    ],
  },

  Business: {
    center: new THREE.Vector3(0, 7.5, -1),
    scale: 2.5,
    svgString: SVGS.CHART,
    radius: 4,
    color: '#44ff88',
    starPoints: [
      { x: -0.8, y: -0.8 },
      { x: -0.8, y: 0.8 },
      { x: 0.8, y: -0.8 },
      { x: -0.3, y: -0.3 },
      { x: 0.2, y: 0.2 },
      { x: 0.7, y: 0.6 },
    ],
  },

  Systems: {
    center: new THREE.Vector3(0, 0.5, 0),
    scale: 2.8,
    svgString: SVGS.GEAR,
    radius: 4.5,
    color: '#ffaa44',
    starPoints: [
      { x: 0, y: 0.9 },
      { x: 0.6, y: 0.6 },
      { x: 0.9, y: 0 },
      { x: 0.6, y: -0.6 },
      { x: 0, y: -0.9 },
      { x: -0.6, y: -0.6 },
      { x: -0.9, y: 0 },
      { x: -0.6, y: 0.6 },
    ],
  },

  Tooling: {
    center: new THREE.Vector3(0, -5.5, 0),
    scale: 2.4,
    svgString: SVGS.TOOLS,
    radius: 4,
    color: '#ffff00',
    starPoints: [
      { x: -0.7, y: 0.6 },
      { x: 0.7, y: -0.6 },
      { x: -0.4, y: 0.3 },
      { x: 0, y: 0 },
      { x: 0.4, y: -0.3 },
    ],
  },

  'Cloud and DevOps': {
    center: new THREE.Vector3(-7.5, -1.5, -0.5),
    scale: 2.5,
    svgString: SVGS.CLOUD,
    radius: 4,
    color: '#ff4444',
    starPoints: [
      { x: -0.9, y: -0.3 },
      { x: -0.5, y: 0.3 },
      { x: 0, y: 0.6 },
      { x: 0.5, y: 0.3 },
      { x: 0.9, y: -0.3 },
      { x: 0, y: -0.4 },
    ],
  },

  Core: {
    center: new THREE.Vector3(7, -1.5, -0.5),
    scale: 2.5,
    svgString: SVGS.LIGHTBULB,
    radius: 4,
    color: '#4488ff',
    starPoints: [
      { x: 0, y: 0.8 },
      { x: 0.5, y: 0.4 },
      { x: -0.5, y: 0.4 },
      { x: 0.3, y: -0.2 },
      { x: -0.3, y: -0.2 },
      { x: 0, y: -0.8 },
    ],
  },
};

function preProcessSVG(svgString: string) {
  const loader = new SVGLoader();
  const data = loader.parse(svgString);
  const lineGeometries: THREE.BufferGeometry[] = [];

  // 1. First, find the bounding box of the ENTIRE SVG to normalize it as a whole
  const totalBBox = new THREE.Box3();

  data.paths.forEach((path) => {
    path.subPaths.forEach((subPath) => {
      const points = subPath.getPoints();
      const geometry = new THREE.BufferGeometry().setFromPoints(points);
      geometry.computeBoundingBox();
      if (geometry.boundingBox) totalBBox.union(geometry.boundingBox);
    });
  });

  const size = new THREE.Vector3();
  totalBBox.getSize(size);
  const centerOffset = new THREE.Vector3();
  totalBBox.getCenter(centerOffset);

  // 2. Calculate a normalization factor (fitting the largest dimension into 1 unit)
  const maxDim = Math.max(size.x, size.y);
  const normalizationScale = 1 / (maxDim || 1);

  // 3. Apply the transformation to each sub-path geometry
  data.paths.forEach((path) => {
    path.subPaths.forEach((subPath) => {
      const points = subPath.getPoints();
      const geometry = new THREE.BufferGeometry().setFromPoints(points);

      // Center based on the TOTAL SVG center, not just the subpath center
      geometry.translate(-centerOffset.x, -centerOffset.y, -centerOffset.z);

      // Scale down to 1x1 unit size
      geometry.scale(normalizationScale, normalizationScale, normalizationScale);

      lineGeometries.push(geometry);
    });
  });

  return lineGeometries;
}

// Transform the raw definitions into 3D-ready data.
// We map the record to an array for easier iteration in the components.
export const PROCESSED_CONSTELLATIONS = Object.entries(CONSTELLATIONS).map(([name, def]) => ({
  name,
  ...def,
  geometries: preProcessSVG(def.svgString),
}));
