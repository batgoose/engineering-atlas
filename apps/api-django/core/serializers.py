...
class CommitCodeReferenceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'repository', 'file_path', 'start_line', 'end_line', 'language', 'github_url', 'raw_url', 'cached_snippet']
...
class CompetencySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Competency
        fields = [
            'id', 'name', 'category', 'competency_type', 'proficiency', 
            'summary', 'tags', 'sub_competencies', 'related_competencies', 
            'showcase_priority', 'portfolio_highlight', 'history'
        ]
...
class ArtifactSerializer(serializers.ModelSerializer):
    ...
    competencies = ArtifactCompetencySerializer(source='artifactcompetency_set', many=True, read_only=True)
    
    class Meta:
        model = Artifact
        fields = [
            'id', 'title', 'status', 'complexity', 'demo_type', 
            'description', 'tech_stack', 'repo_url', 'live_url', 
            'date_created', 'competencies'
        ]