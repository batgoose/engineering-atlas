    ...
    def seed_competencies(self, file_path):
        ...
        for item in data:
            ...
            defaults = {
                'name': item['name'],
                'category': category,
                'competency_type': item.get('competency_type', 'concept'), # Defaults to concept
                'proficiency': item['proficiency'],
                'summary': item.get('summary', ''),
                'tags': item.get('tags', []),
                'showcase_priority': item.get('showcasePriority', 'medium'), # Handles camelCase if present
            ...
            if 'related_ids' in item:
                ...
                for rel_id in item['related_ids']:
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f"Warning: Related ID '{rel_id}' not found for '{item['id']}'"))

        self.stdout.write(self.style.SUCCESS(f"  Created {created_count}, Updated {updated_count} competencies"))
    ...
    def seed_artifacts(self, file_path):
        ...
        for item in data:
            ...
            defaults = {
                ...
                'repo_url': item.get('repo_url', ''), # Critical: Matches snake_case JSON
            ...
            if 'competencies' in item:
                ...
                for comp_data in item['competencies']:
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f"  Skill '{comp_data['id']}' not found for artifact '{item['id']}'"))
