# api-django/core/management/commands/seed_data.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Path logic to find seeds - works in both Docker and local dev
        search_paths = [
            '/app/seeds',  # Docker mounted path
            os.path.join(settings.BASE_DIR.parent.parent, 'packages/db/seeds'),  # Local dev
        ]
    ...
    def seed_artifacts(self, path):
        ...
        for item in data:
            # Replaced 'complexity' with 'domain' to match your architectural update
            artifact, _ = Artifact.objects.update_or_create(
                id=item['id'],
                defaults={
                    'title': item.get('title', item['id']),
                    'status': item.get('status', 'planned'),
                    'domain': item.get('domain', 'atlas'), # Default to 'atlas' domain
            ...
            # Link Competencies via through model
