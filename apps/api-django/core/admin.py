...
from .models import Category, Competency, SubCompetency, Artifact, ArtifactCompetency, CommitCodeReference
...
class CompetencyAdmin(admin.ModelAdmin):
    list_display = ('name', 'category', 'competency_type', 'proficiency', 'showcase_priority', 'portfolio_highlight')
    list_filter = ('category', 'competency_type', 'proficiency', 'showcase_priority', 'portfolio_highlight')
    ...
    fieldsets = (
        ('Basic Info', {
            'fields': ('id', 'name', 'category', 'competency_type', 'proficiency', 'summary')
        }),
        ('Metadata', {
            'fields': ('tags', 'related_competencies', 'history')
        }),
        ('Display', {
            'fields': ('showcase_priority', 'portfolio_highlight')
        }),
    )
...
class CommitCodeReferenceAdmin(admin.ModelAdmin):
    list_display = ('file_path', 'commit_hash_short', 'language', 'line_range', 'created_at')
    ...
    fieldsets = (
        ...
        ('Code Location', {
            'fields': ('start_line', 'end_line', 'language')
        }),
        ('Cache', {
            'fields': ('cached_snippet',),
            'classes': ('collapse',)
        }),
        ('URLs', {
...
class ArtifactAdmin(admin.ModelAdmin):
    list_display = ('title', 'status', 'complexity', 'demo_type', 'primary_competencies_list', 'date_created')
    ...
    fieldsets = (
        ('Basic Info', {
            'fields': ('id', 'title', 'description', 'status', 'complexity', 'demo_type')
        }),
        ('Links', {
    ...
    def get_form(self, request, obj=None, **kwargs):
            ...
            field.help_text = "Enter technologies separated by commas (e.g. 'Python, React, AWS')"
