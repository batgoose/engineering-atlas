"""
Gridstream Redis caching layer.
...
Cache keys follow the pattern: gridstream:{resource}:{identifier}:{params_hash}
"""
...
def cache_key(resource: str, identifier: str = "", params: dict | None = None) -> str:
    ...
    Examples:
      cache_key("games", "2025_01")           → "gridstream:games:2025_01"
      cache_key("standings", params={...})     → "gridstream:standings:a1b2c3d4e5"
      cache_key("news", "SEA", {"page": 1})   → "gridstream:news:SEA:f6e7d8c9b0"
    """
    parts = ["gridstream", resource]
    if identifier:
...
def invalidate_game(game_id: int) -> None:
    ...
    cache_delete_pattern(f"gridstream:games:{game_id}*")
    cache_delete_pattern(f"gridstream:game_plays:{game_id}*")
    cache_delete_pattern(f"gridstream:game_drives:{game_id}*")
    cache_delete_pattern("gridstream:standings*")


def invalidate_team(abbreviation: str) -> None:
    """Clear cached data for a team."""
    cache_delete_pattern(f"gridstream:teams:{abbreviation}*")
    cache_delete_pattern(f"gridstream:roster:{abbreviation}*")
    cache_delete_pattern("gridstream:standings*")


def invalidate_player(player_id: int) -> None:
    """Clear cached data for a player."""
    cache_delete_pattern(f"gridstream:players:{player_id}*")
    cache_delete_pattern(f"gridstream:gamelog:{player_id}*")
