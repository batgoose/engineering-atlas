"""
Import nfldata draft picks into raw.raw_nflverse_draft_picks.
...
class Command(ImportBaseCommand):
    help = "Import nfldata draft picks into raw.raw_nflverse_draft_picks."
    ...
    def handle(self, *args, **options):
        ...
        self._populate_pick_in_round(rows)

        self.stdout.write(
            f"Importing draft picks for {len(target_seasons)} seasons: "
    ...
    def _ensure_required_tables(self):
        with self.get_nfl_cursor() as cursor:
            cursor.execute("""
                SELECT
                    to_regclass('raw.raw_nflverse_draft_picks'),
                    to_regclass('raw.raw_ingest_batch')
                """)
            draft_picks_table, ingest_batch_table = cursor.fetchone()

        if not draft_picks_table:
            raise CommandError(
                "Missing table raw.raw_nflverse_draft_picks. Run migrations first."
            )
    ...
    def _download_source(self):
        response = requests.get(
            SOURCE_URL,
            timeout=180,
            headers={"User-Agent": "engineering-atlas/import_nflverse_draft_picks"},
        )
    ...
    def _parse_rows(self, content):
        ...
        for row in reader:
            ...
            round_number = self.safe_int(row.get("round"))
            overall_pick = self.safe_int(row.get("pick"))
            if season is None or round_number is None or overall_pick is None:
                continue
        ...
        rows.sort(key=lambda r: (r["season"], r["round"], r["overall_pick"]))
        return rows

    def _populate_pick_in_round(self, rows):
        counters = defaultdict(int)
        for row in rows:
            key = (row["season"], row["round"])
            counters[key] += 1
            row["pick_in_round"] = counters[key]
    ...
    def _resolve_target_seasons(self, requested, seasons_in_file):
        if requested:
            ...
            if missing:
                self.stdout.write(
                    self.style.WARNING(
                        f"Requested seasons not in draft_picks.csv: {missing}"
                    )
                )
    ...
    def _begin_batch(self, seasons, checksum):
        metadata = json.dumps(
            {
                "status": "started",
                "ingest_tool": "django.import_nflverse_draft_picks",
                "target_table": "raw.raw_nflverse_draft_picks",
    ...
    def _delete_existing_rows(self, seasons):
        ...
        with self.get_nfl_cursor() as cursor:
            cursor.execute(
                "DELETE FROM raw.raw_nflverse_draft_picks WHERE season = ANY(%s)",
                [seasons],
            )
